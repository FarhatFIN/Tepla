...
const messageCursors = new Map<string, string | null>(); // chatId -> nextCursor for history pagination
...
export const useChatStore = create<ChatState>((set, get) => ({
  ...
  loadFolders: async () => {
    try {
      const res = await api.get<{ success: boolean; data: any[] }>("/folders");
      ...
      console.warn("[chat-store] loadFolders failed:", err?.message || err);
    }
  },

  createFolder: async (name: string, icon?: string) => {
    try {
      await api.post("/folders", { name, emoji: icon });
      await get().loadFolders();
    } catch (err: any) {
      console.warn("[chat-store] createFolder failed:", err?.message || err);
    }
  },
  ...
  deleteFolder: async (folderId: string) => {
    ...
    try {
      ...
      console.warn("[chat-store] deleteFolder failed:", err?.message || err);
      get().loadFolders();
    }
  },

  assignChatsToFolder: async (folderId: string, chatIds: string[]) => {
    set((s) => ({
      folders: s.folders.map((f) => (f.id === folderId ? { ...f, chatIds } : f)),
    }));
    try {
      await api.put(`/folders/${folderId}/chats`, { chatIds });
    } catch (err: any) {
  ...
  loadOlderMessages: async (chatId: string) => {
    const cursor = messageCursors.get(chatId);
    if (!cursor) return;
    try {
      const res = await api.get<{ success: boolean; data: any[]; meta: any }>(
      ...
      console.warn("[chat-store] loadOlderMessages failed:", err?.message || err);
    }
  },
  ...
  bindSocket: () => {
    ...
    socket.on("message:updated", (data: { chatId: string; message?: any; messageId?: string; content?: string }) => {
      // Backend sends { chatId, message }; older shape { messageId, content } is also supported
      const updated = data.message ? mapBackendMessage({ ...data.message, chat_id: data.chatId }) : null;
      set((s) => ({
        messages: {
          ...s.messages,
          [data.chatId]: (s.messages[data.chatId] || []).map((m) => {
            if (updated && m.id === updated.id) return { ...m, text: updated.text, isEdited: true };
            if (!updated && m.id === data.messageId) return { ...m, text: data.content ?? m.text, isEdited: true };
            return m;
          }),
        },
      }));
    ...
    socket.on("reaction:changed", (data: { chatId: string; messageId: string; reactions?: Array<{ emoji: string; count: number }> }) => {
      set((s) => ({
        messages: {
          ...s.messages,
          [data.chatId]: (s.messages[data.chatId] || []).map((m) =>
            m.id === data.messageId
              ? {
                  ...
                  reactions: (data.reactions || []).map((r) => ({
                    ...
                    count: r.count,
                    users: [],
                    myReaction: m.reactions?.find((x) => x.emoji === r.emoji)?.myReaction || false,
                  })),
                }
              : m
          ),
  ...
  addReaction: (chatId, messageId, emoji) => {
    // Optimistic toggle; authoritative counts arrive via reaction:changed
    set((s) => ({
      messages: {
        ...s.messages,
        [chatId]: (s.messages[chatId] || []).map((m) => {
    ...
    api.post("/reactions", { messageId, emoji }).catch((err) =>
      console.warn("[chat-store] addReaction failed:", err)
    );
  },

  markAsRead: (chatId) => {
    set((s) => ({ chats: s.chats.map((c) => c.id === chatId ? { ...c, unreadCount: 0 } : c) }));
    // Notify server with the actual message ids (required by the API)
    const myId = useAuthStore.getState().user?.id;
    const messageIds = (get().messages[chatId] || [])
      .filter((m) => m.senderId !== myId && !m.id.startsWith("tmp-"))
      .slice(-100)
      .map((m) => m.id);
    if (messageIds.length === 0) return;
    api.post("/messages/read", { chatId, messageIds }).catch((err) =>
  ...
  editMessage: (chatId, messageId, text) => {
    ...
    api.patch(`/messages/${messageId}`, { content: text }).catch((err) =>
      console.warn("[chat-store] editMessage failed:", err)
    );
  },

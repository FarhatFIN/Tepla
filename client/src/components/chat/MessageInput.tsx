...
export default function MessageInput({ chatId }: MessageInputProps) {
  ...
  const attachOptions = [
    { icon: <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><rect x="3" y="3" width="18" height="18" rx="2"/><circle cx="8.5" cy="8.5" r="1.5"/><polyline points="21 15 16 10 5 21"/></svg>, label: t("photo_video"), color: "text-[#8B5CF6]", action: () => mediaInputRef.current?.click() },
    { icon: <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/></svg>, label: t("file"), color: "text-[#C4B5FD]", action: () => fileInputRef.current?.click() },
    { icon: <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0 1 18 0z"/><circle cx="12" cy="10" r="3"/></svg>, label: t("location"), color: "text-[#00D46A]", action: () => setShowLocation(true) },
    { icon: <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M12 20V10"/><path d="M18 20V4"/><path d="M6 20v-4"/></svg>, label: t("poll"), color: "text-[#F59E0B]", action: () => setShowPoll(true) },
    { icon: <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><rect x="2" y="2" width="20" height="20" rx="5"/><path d="M8 14s1.5 2 4 2 4-2 4-2"/></svg>, label: t("gif"), color: "text-[#6C3DE8]", action: () => { setShowGif(true); searchGifs(); } },
  ];
              ...
              {showAttach && (
                <div className="absolute bottom-12 left-0 z-20 w-48 rounded-xl bg-[var(--bg-card)] p-1.5 shadow-lg animate-scale-in">
                  {attachOptions.map((opt) => (
                    <button key={opt.label} onClick={() => { setShowAttach(false); opt.action(); }} className="flex w-full items-center gap-3 rounded-lg px-3 py-2 text-left text-sm transition-colors hover:bg-[var(--bg-hover)]">

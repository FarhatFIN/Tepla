...
const logger = createLogger('translation-module');
...
// ─── Router ───────────────────────────────
export function translationRouter(redis: RedisClient, kafka: KafkaProducer): Router {
  const router = Router();
  ...
  router.post('/', authMiddleware(), async (req, res, next) => {
    try {
      ...
      // Emit event
      await kafka.send(EventTopic.TRANSLATION_EVENTS, {
  ...
  router.post('/batch', authMiddleware(), async (req, res, next) => {
    try {
      ...
      // Increment daily counter
      await redis.incrby(dailyKey, messages.length);
      await redis.expire(dailyKey, 86400);
  ...
  router.post('/auto', authMiddleware(), async (req, res, next) => {
    try {
      ...
      if (enabled) {
        await redis.set(key, targetLang || 'auto');
      } else {
        await redis.del(key);
      }
  ...
  router.get('/auto/:chatId', authMiddleware(), async (req, res, next) => {
    try {
      const key = `translate:auto:${req.user!.sub}:${req.params.chatId}`;
      const lang = await redis.get(key);

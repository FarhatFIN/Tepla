...
export class SecurityMetrics {
  ...
  static async authFailure(redis: Redis): Promise<void> {
  ...
  static async authSuccess(redis: Redis): Promise<void> {
    const today = new Date().toISOString().split('T')[0];
    await redis.incr(`sec_metric:auth_success:${today}`);
  }

  static async replay(redis: Redis): Promise<void> {
    const today = new Date().toISOString().split('T')[0];
  ...
  static async rateLimit(redis: Redis): Promise<void> {
    const today = new Date().toISOString().split('T')[0];
  ...
  static async encryptionError(redis: Redis): Promise<void> {
    await redis.incr('sec_metric:encryption_error:total');
  }

  static async anomalyDetected(redis: Redis): Promise<void> {
    const today = new Date().toISOString().split('T')[0];
  ...
  static async getAll(redis: Redis): Promise<Record<string, number>> {
    const today = new Date().toISOString().split('T')[0];
    const keys = [
    ...
    keys.forEach((key, i) => {
      ...
      metrics[name] = parseInt((results?.[i]?.[1] as string) || '0');
    });
